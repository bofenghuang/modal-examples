...
# GPU_CONFIG = modal.gpu.A100(memory=80, count=6)
...
def main():
    ...
    def _generate(user_question):
        tokens = []
        for _, token_text in enumerate(
            Model().completion_stream.remote_gen(
                # messages=messages,
    ...
    print(_generate("Lequel est plus lourd entre 1 kilogramme de fer et 1 kilogramme de coton ?"))

    run_generation(gen_func=_generate)
