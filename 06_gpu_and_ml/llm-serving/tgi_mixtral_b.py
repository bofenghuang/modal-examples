...
LAUNCH_FLAGS = [
    ...
    "6144",
    # "2048",
    # The larger this value, the larger amount each request will be in your RAM and the less effective batching can be.
    "--max-total-tokens",
    "8192",
    # "4096",
    "--max-batch-prefill-tokens",  # Default to `max_input_tokens + 50`
    "8192"
]
...
tgi_image = (
    ...
    # Image.from_registry("ghcr.io/huggingface/text-generation-inference:sha-59b3ffe")
...
@app.cls(
    secrets=[Secret.from_name("huggingface-secret")],
    gpu=GPU_CONFIG,
    concurrency_limit=1,  # prevent autoscaling of containers
    # allow_concurrent_inputs=10,
    # allow_concurrent_inputs=16,
    allow_concurrent_inputs=32,  # capacity of a single container to handle some number of simultaneous input
...
class Model:
    ...
    def start_server(self):
        ...
        self.client = AsyncClient("http://127.0.0.1:8000", timeout=60 * 5)
    ...
    async def generate(
        ...
        # # todo: don't pass bos since it will be added later automatically
        # prompt = prompt.replace("<s> ", "", 1)

        # result = await self.client.generate(prompt, max_new_tokens=1024)
        # result = await self.client.generate(prompt, max_new_tokens=1024, do_sample=False)
...
def main():
    ...
    #         "Lequel est plus lourd entre 1 kilogramme de fer et 1 kilogramme de coton ?",
    #     )
    # )

    # print(
    #     model.generate_chat.remote(
    #         [{"role": "user", "content": "Lequel est plus lourd entre 1 kilogramme de fer et 1 kilogramme de coton ?"}],
    #     )
    # )
    ...
    # from gen_grade_instruct import run_generation
    # from gen_instruct import run_generation
    # from gen_translation import run_generation

    # from gen_multi_turn_question import run_generation
    from gen_response import run_generation

    # run_generation(gen_func=model.generate.remote)
    run_generation(gen_func=model.generate_chat.remote)
